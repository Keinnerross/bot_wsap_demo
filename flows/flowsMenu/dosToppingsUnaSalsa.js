...
//variables
const bodyEntry = 3; // entrada del usuario, es decir, la opción que elige.
...
...salsas.map((salsa, index) => `${index + 1}\u20E3 ${salsa}`)
    ...
    if (ctx.body >= 1 && ctx.body <= adicionales.toppingsClasicos.length) {
        ...
        await state.update({ toppings: toppingChoosed }); // Guarda salsa seleccionada
        const myState = state.getMyState();
...
}).addAnswer("Escoge un *segundo Topping*", {
    capture: true
}, async (ctx, { flowDynamic, state, fallBack }) => {
    ...
    if (ctx.body >= 1 && ctx.body <= adicionales.toppingsClasicos.length) {
        ...
        const toppingChoosed = adicionales.toppingsClasicos[ctx.body - 1].topping;
        const myStateBefore = state.getMyState();
        await state.update({ toppings: myStateBefore.toppings + ", " + toppingChoosed });
        const myStateAfter = state.getMyState();


        await flowDynamic(`*Toppings elegidos:* ${myStateAfter.toppings}`)
    } else {
        return fallBack();
    }
...
module.exports = dosToppingsUnaSalsa;
